...
Citizen.CreateThread(function()
	while true do
			...
			if(NetworkIsPlayerActive(i) and GetPlayerPed(i) ~= GetPlayerPed(-1))then
				if(HasEntityClearLosToEntity(GetPlayerPed(-1), GetPlayerPed(i), 17) and IsEntityVisible(GetPlayerPed(i)))then
					...
					if(Vdist(pos.x, pos.y, pos.z, posme.x, posme.y, posme.z) < 10.0)then
						...
						SetTextEntry("STRING");
						SetTextCentre(1)
						AddTextComponentString(GetPlayerServerId(i) .. " " )
						DrawText(y, z)
		...
		if(var2 ~= 0)then
			drawTxt(0.555, 1.25, 1.0,1.0,0.4, "Crossing " .. tostring(GetStreetNameFromHashKey(var2)) .. " ", 255, 255, 255, 255)
		end

		if(GetStreetNameFromHashKey(var1) and GetNameOfZone(pos.x, pos.y, pos.z))then
			if(zones[GetNameOfZone(pos.x, pos.y, pos.z)] and tostring(GetStreetNameFromHashKey(var1)))then
				drawTxt(0.555, 1.22, 1.0,1.0,0.4, " " .. tostring(GetStreetNameFromHashKey(var1)) .. " in " .. zones[GetNameOfZone(pos.x, pos.y, pos.z)], 255, 255, 255, 255)
			end
		end
			...
			for i = 0,32 do
				if(GetPlayerName(i))then
					if(NetworkIsPlayerTalking(i))then
						...
						if(t == 1)then
								drawTxt(0.515, 0.95, 1.0,1.0,0.4, "~y~En train de parler", 255, 255, 255, 255)
						end

						drawTxt(0.520, 0.95 + (t * 0.023), 1.0,1.0,0.4, GetPlayerServerId(i) .. "  " , 255, 255, 255, 255)
					end
				end
			end
